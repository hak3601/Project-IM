...
if uploaded:
    ...
    if img_bgr is None:
        ...
        # filename 기반(s_/f_) 강제 모드 적용됨
        results = analyze_face_regions(img_bgr, filename=uploaded.name)

        if not results:
            st.error("얼굴을 찾지 못했습니다. 정면 또는 측면 얼굴이 잘 보이도록 다시 업로드해주세요.")
        else:
            for region_name, res in results.items():
                ...
                if show_debug and ("debug" in res):
                    dbg = res["debug"]
                    with st.expander("🧪 Debug masks (skin/red/smooth/separated)"):
