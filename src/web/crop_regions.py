...
def crop_side(img, pts):
    """
    Side-face: use bbox logic.
    Returns:
      - 'forehead'
      - 'left_cheek' OR 'right_cheek' (보이는 쪽 하나만)
    """
    h, w = img.shape[:2]
    regions = {}
    ...
    fh_pts = pts[FOREHEAD_SIDE_IDXS]  # SIDE용 index 세트 사용
    x_min_fh = int(np.min(fh_pts[:, 0]))
    x_max_fh = int(np.max(fh_pts[:, 0]))
    y_top_fh = int(np.min(fh_pts[:, 1]))
